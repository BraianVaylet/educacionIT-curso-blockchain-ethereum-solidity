...
contract Instituto {
    ...
    function llamarSumCalculadora(
        address contrato,
        uint256 num1,
        uint256 num2
    ) private returns (uint256) {
        (bool resultado, bytes memory respuesta) = contrato.call(
            abi.encodeWithSignature("sumar(uint256, uint256)", num1, num2)
        );
        return abi.decode(respuesta, (uint256));
    }
